...
export const paymentPlanCards: Record<PaymentPlanId, PaymentPlanCard> = {
  [PaymentPlanId.Small]: {
    ...
    price: "1000Ft",
    description: "",
    features: ["Bárhol felhasználható"],
  },
        ...
        <div className="isolate mx-auto mt-16 grid max-w-md grid-cols-1 gap-y-8 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-x-8">
          {Object.values(PaymentPlanId).map((planId) => (
            ...
            >
              ...
              <CardContent className="h-full text-center justify-between p-8 xl:p-10">
                <div className="flex items-center justify-between gap-x-4">
                  <CardTitle
                    id={planId}
                    className="text-foreground text-center text-lg leading-8 font-semibold"
                  >
                    {paymentPlanCards[planId].name}
                  </CardTitle>
                </div>
                <p className="text-muted-foreground text-center mt-4 text-sm leading-6">
                  {paymentPlanCards[planId].description}
                </p>
                <p className="mt-6 flex text-center items-baseline gap-x-1">
                  <span className="text-foreground text-center text-3xl font-bold ">
                    {paymentPlanCards[planId].price}
                  </span>
                  <span className="text-muted-foreground text-center text-sm leading-6 font-semibold">
                    {paymentPlans[planId].effect.kind === "credits"}
                  </span>
                </p>
                <ul
                  role="list"
                  className="text-muted-foreground mt-8 space-y-3 text-center text-sm leading-6"
                >
                  {paymentPlanCards[planId].features.map((feature) => (
                    <li key={feature} className="flex gap-x-3">
                      <CheckCircle
                        className="text-primary text-center h-5 w-5 flex-none"
                        aria-hidden="true"
                      />
                      {feature}
                    </li>
